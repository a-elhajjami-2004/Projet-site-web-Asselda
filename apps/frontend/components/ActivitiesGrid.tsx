...
export default function ActivitiesGrid({ albums: activities, lang }: { albums: Promise<any>; lang: Language }) {
			...
			{activitiesResponse.data ? (
				activitiesResponse.data.map(
					({
						...
						<div className="bg-lime-100 flex flex-col rounded-lg overflow-hidden shadow-lg/30 max-w-sm">
							<img
							...
							<div className="p-6 flex-1">
								<span className="inline-block bg-[#49a120] text-white text-xs font-bold px-3 py-1 rounded mb-4">
									{/*getTranslation(lang, `activities.domains.${domain}`)*/}
									Environnement
								</span>
